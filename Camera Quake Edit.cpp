...
Bool Update()
{
   ...
   if(Kb.bp(KB_SPACE))qe.addMedium(); // add medium quake camera effect when space pressed
	...
	if(Kb.b(KB_1))      qe.addLong(); // add medium quake camera effect when space pressed
                      qe.update   (); // update           camera effects
	if(Kb.bp(KB_LCTRL))qe.addShort(); // add medium quake camera effect when space pressed
                      qe.update   (); // update           camera effects
...
void Draw()
{
   ...
   REPD(x, 5) {
	   REPD(z, 5) {
			if (x % 2) {
				if (z % 2)
					Box(0.3f, Vec(x-2, 0, z-2)).draw(RED, true);
				else
					Box(0.4f, Vec(x-2, 0, z-2)).draw(BLUE, true);
			}
			else {
				if (z % 2)
					Tube(0.3f, 0.8f, Vec(x-2, 0, z-2)).draw(YELLOW, true);
				else
					Tube(0.4f, 0.9f, Vec(x-2, 0, z-2)).draw(GREEN, true);
			}
	   }
   }
